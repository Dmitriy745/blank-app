...
with col2:
    ...
    if not df.empty:
        ...
        fig = px.line(
            ...
            x='дата', 
            y='сумма',
            title='Динамика расходов по датам',
            labels={'дата': 'Дата', 'сумма': 'Сумма (руб.)'},
            markers=True
        )
        ...
        with col_stat2:
            ...
            st.metric("Общая сумма", f"{total_amount:.2f} руб.")
        
        with col_stat3:
            avg_amount = df['сумма'].mean()
            st.metric("Средняя сумма", f"{avg_amount:.2f} руб.")
...
# Футер
st.markdown("---")
st.markdown("*Приложение для учета расходов*")
